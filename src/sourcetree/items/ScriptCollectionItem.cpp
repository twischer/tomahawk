...
ScriptCollectionItem::isBeingPlayed() const
{
    if ( m_page )
    {
        ...
        if ( !m_page->playlistInterface().isNull() &&
             m_page->playlistInterface() == MainAudioEngine::instance()->currentTrackPlaylist() )
            return true;

        if ( !m_page->playlistInterface().isNull() &&
             m_page->playlistInterface()->hasChildInterface( MainAudioEngine::instance()->currentTrackPlaylist() ) )
            return true;
    }
    return false;
}
